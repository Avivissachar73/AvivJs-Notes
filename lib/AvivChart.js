...
export class BaseDataChart extends BaseChart {
  ...
  get textStyle() {
    return { fillStyle: 'black', font: 'sans-serif' };
  }
  ...
  get extraPadForData() {
    return { top: 15, bottom: 30, right: 15, left: 30 };
  }

  get totalPad() {
    const { padding: pad, extraPadForData: exPAd } = this;
    return {
      top: pad.top + exPAd.top,
      bottom: pad.bottom + exPAd.bottom,
      right: pad.right + exPAd.right,
      left: pad.left + exPAd.left
    }
  }

  static calcSectionRenderPos(item, size, {position, align}) {
    const pos = {
      position, align,
      x: size.w/2 - item.w/2,
      y: size.h/2 - item.h/2,
    ...
    const { main: mainSizeKey, sec: secSizeKey } = this.sizeKeyByAlignOrAxis(position);
    const { main: mainAxis, sec: secAxis } = this.axisBySizeKeyOrAlign(position);
    ...
    switch (align) {
      case 'start':
        ...
        break;
      case 'end':
        pos[secAxis] = size[secSizeKey] - item[secSizeKey];
        break;
      case 'middle':
      case 'center':
        break;
  ...
  get infoSectionsPos() {
    const { size, infoSectionSize, legendSectionSize } = this;
    return {
      info: this.constructor.calcSectionRenderPos(infoSectionSize, size, {
        position: this.chartData.info.position, align: this.chartData.info.align
      }),
      legend: this.constructor.calcSectionRenderPos(legendSectionSize, size, {
        position: this.chartData.legend?.position, align: this.chartData.legend?.align
      }),
  ...
  get infoSectionSize() {
    ...
    const lineSize = UTILS.getTextPxSize(longestLine, lineHeight);
    return {
      lineHeight,
      gap,
      h: totalHeight,
  ...
  get legendSectionSize() {
    ...
    return { h, w: UTILS.getTextPxSize(this.legend?.tag, h) }; 
  }
...
const UTILS = {
  ...
  getTextPxSize: function(text, size) {
    const _context = document.createElement('canvas').getContext('2d');
    _context.font = `${size}px Ariel`;
