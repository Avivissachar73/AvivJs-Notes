...
function getDefaultChartData() {
  const defaultChartData = { 
    data: [
      // {tag: 'dataSample1', vals: [10, 73, 40]}, style: {color: '#abcdeb', strokeStyle: 'gold'},
      // {tag: 'dataSample2', vals: [50, -50, 30]}, style: {color: '#fdaaaa', strokeStyle: 'cilver'}
    ], 
    width: 1600,
    height: 900,
    // labels: ['sep', 'oct', 'nov'],
...
export class BaseDataChart extends BaseChart {
  ...
  constructor(chartData = getDefaultChartData(), ...args) {
    super(UTILS.fillData(chartData, getDefaultChartData(), ['legend']), ...args);
  ...
  get textStyle() {
    return { strokeStyle_: 'black', fillStyle: 'black', fontWeight: '600', fontFamily: 'sans-serif', size: 40, ...(this.chartData.textStyle || {}) };
  }
  ...
  getGridRenderData() {
    ...
    // chart outline
    const gridStyle = { strokeStyle: 'gray', lineWidth: 3, w: 1, h:1 };
...
export class LineChart extends BaseDataChart {
  getCahrtDataToRender(data) {
    ...
    return [
      ...
      ...data.map((curr, idx) => ({
        style: { strokeStyle: this.constructor.getClrFromStyle(curr.style, idx), lineWidth: this.chartData.style.lineWidth || 5, lineCap: 'round' },
      ...
      ...data.reduce((acc, curr, idx) => {
        acc.push(
          ...curr.vals.map((val, i) => ({
            ...
            style: { fillStyle: this.constructor.getClrFromStyle(curr.style, idx), strokeStyle: this.chartData.style?.strokeStyle, lineWidth: this.chartData.style?.lineWidth },
...
export class PiChart extends BaseDataChart {
  constructor(chartData = getBasePiChartData(), ...args) {
    super(UTILS.fillData(chartData, getBasePiChartData(), ['legend']), ...args);
  }
  ...
  get strokeLineWidth() {
    return this.chartData.style?.lineWidth || 7.5;
  }
  get baseCircleItem() {
    return {
      ...this.piPos,
      ...
      h: this.piSize,
      circleDegries: 360,
      style: { strokeStyle: this.chartData.style?.strokeStyle || 'white', lineWidth: this.strokeLineWidth },
    }
  }
  getCahrtDataToRender() {
    const { chartData } = this;
    const valsSum = chartData.data.reduce((acc, c) => acc + (this.constructor.getItemVal(c)), 0);
    // const baseDegs = this.chartData.piDegries || [0, 360];
    const totalDegs = this.chartData.totalPiDegries || 360;
    // const totalDegs = Math.abs(baseDegs[1] - baseDegs[0]);
    // let lastEndDeg = 180;
    let lastEndDeg = this.chartData.paintFromDegrie;
    return [
      ...
      ...chartData.data.map((curr, idx) => {
        ...
        const endDeg = startDeg + totalDegs*percentages;
        lastEndDeg = endDeg;
        return {
          ...this.baseCircleItem,
          w: this.baseCircleItem.w - this.strokeLineWidth/2,
          h: this.baseCircleItem.h - this.strokeLineWidth/2,
          circleDegries: [startDeg, endDeg],
          closeCircle: true,
          style: { ...this.baseCircleItem.style, fillStyle: this.constructor.getClrFromStyle(curr.style, idx), strokeStyle: curr.style.strokeStyle || this.baseCircleItem.style.strokeStyle },
  ...
  getAnimationFrames() {
    ...
    for (let i = 0; i < animationSteps.length; i++) {
      const currFrame = finalData.map(c => {
        ...
        return {
          ...c,
          circleDegries: c.circleDegries ? (Array.isArray(c.circleDegries)? c.circleDegries.map(c => c*animationSteps[i]) : c.circleDegries*animationSteps[i]) : undefined
        }
      });
...
function getBaseDonatChartData() {
  return {
    ...getBasePiChartData(),
    donatWidthPercents: 50
  }
}
export class DonatChart extends PiChart {
  constructor(chartData = getBaseDonatChartData(), ...args) {
    super(UTILS.fillData(chartData, getBaseDonatChartData(), ['legend']), ...args);
  }
  get donatHoleSize() {
    // return this.piSize/2;
    return this.piSize - this.piSize * ((this.chartData.donatWidthPercents || 50) / 100);
  }
  ...
  getCahrtDataToRender() {
    return [
      ...super.getCahrtDataToRender(),
      {
        ...
        ...this.centerCircle,
        closeCircle: true,
        // circleDegries: [this.chartData.paintFromDegrie, this.chartData.totalPiDegries],
        style: { clear: true }
      },
      { ...this.centerCircle, circleDegries: [this.chartData.paintFromDegrie, this.chartData.totalPiDegries - this.chartData.paintFromDegrie], style: { strokeStyle: this.chartData.style?.strokeStyle || 'white', lineWidth: this.strokeLineWidth } }
    ]
  }
}
