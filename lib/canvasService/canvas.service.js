...
import { Utils as utilsLib } from "../utils.service.js";
...
```java script
 const RenderOpts = {
    ...
    style: {      // most props are simply canvas style properties 
        ...
        font: String('Ariel'),
        textAlign: Enumerator('center', 'right', 'left', 'start', 'end'),
        textBaseline: Enumerator('middle', 'top', 'bottom'),
        direction: Enumerator('rtl', 'ltr'),
        gradientBgc: {
            colors: [
    ...
    fontSize: Number,                                       // for text rendernig, default will take w value;
...
export class CanvasService {
    ...
    setup() {
        ...
        const elParent = this.elParent = canvasOpts.selector instanceof Element ? canvasOpts.selector : document.querySelector(canvasOpts.selector);
        ...
        this.elCanvas.style.direction = 'ltr'; // todo:: fix this to be responsive!;
        ...
        for (let evName in this.listeners) {
            ...
            elCanvas.addEventListener(evName, (ev) => {
                ...
                const matchedItems = !this.isMatrixBoard ? 
                    this.board.shapes.filter(c => {
                        const testItem = c.text ? CanvasService.getrenderAreaItemForTxtItem(c) : c;
                        return CanvasService.isPosOnItem(pos, testItem, false);
                    })
                    : [];
                this.listeners[evName](ev, { x: pos.x, y: pos.y }, matchedItems, this);
            });
        }
    ...
    init() {
        ...
        this.elCanvas.width = canvasSize.dprW;
        this.elCanvas.height = canvasSize.dprH;
        this.elCanvas.style.width = canvasSize.w + 'px';
        this.elCanvas.style.height = canvasSize.h + 'px';
        this.context.scale(canvasSize.dpr, canvasSize.dpr);
    ...
    static parseRenderOpts(shape = {}, reqOpts = {}, relativeTo) {
        ...
        const res = {
            ...
            style: shape.style && {
                ...
                // font: `${shape.style.size ? shape.style.size+'px' : '1em'} ${shape.style.font}` || `1em Arial`,
                // font: `20px ${shape.style.font}` || `1em Arial`,
                fontFamily: shape.style.fontFamily, // 'Arial'
                font: shape.style.font, // `1em Arial`
                lineWidth: shape.style.lineWidth || 1,
                strictLineWidth: shape.style.strictLineWidth,
                textAlign: shape.style.textAlign,
                textBaseline: shape.style.textBaseline,
                direction: shape.style.direction,
    ...
    doRenderData(opts, that) {
        ...
        if (opts.img) {
            ...
            if (opts.geoShape) {
                ...
                const fontSize = opts.fontSize || renderW;
                const lines = opts.text.split('\n');
                const longestLine = lines.reduce((acc, c) => c.length > acc.length ? c : acc, '');
                const renderTxtW = Utils.getTextPxSize(longestLine, fontSize);
                // if (style.fontFamily) context.font = (fontSize * 1) + 'px ' + (style.fontFamily || 'Arial');
                // else if (style.font) context.font = style.font;
                if (style.font) context.font = style.font;
                else context.font = (fontSize * 1) + 'px ' + (style.fontFamily || 'Arial');
                // if (context.font) context.font = (renderW * 1.2) + 'px ' + style.font || 'Arial';
                // if (context.font) context.font = '20px ' + style.font || 'Arial';
                if (style.textAlign) {
                    context.textAlign = style.textAlign;
                    if (style.direction === 'rtl') {
                        context.textAlign = Utils.getMirroredAlignValue(context.textAlign);
                    }
                }
                ...
                if (context.textAlign) {
                    if (context.textAlign === 'center') context.moveTo(renderX - renderTxtW/2, renderY);
                    // if (context.textAlign === 'start') context.moveTo(pos.x - renderTxtW/2, pos.y);
                    if (context.direction === 'ltr') {
                        if (context.textAlign === 'end') context.moveTo(renderX - renderTxtW, renderY);
                        // if (context.textAlign === 'start') context.moveTo(renderX + renderTxtW, renderY);
                    } else { // rtl;
                        if (context.textAlign === 'end') context.moveTo(renderX - renderTxtW, renderY);
                        // if (context.textAlign === 'start') context.moveTo(renderX - renderTxtW, renderY);
                    }
                ...
                // ctx.fillText(text, rect.x - m.actualBoundingBoxLeft, rect.y);
                lines.forEach((txtLine, lineIdx, all) => {
                    // const longestLineMesure = all.reduce((acc, c) => Math.max(Math.abs(context.measureText(c).actualBoundingBoxAscent), acc), 0);
                    // let fix = 0;
                    // fix = longestLineMesure*2;
                    // console.log('WOWOW', context.measureText(txtLine));
                    
                    const txtMesure = context.measureText(longestLine);
                    let fix = 0;
                    fix = Math.abs(txtMesure.actualBoundingBoxAscent)*2;
                    // // if (['start', 'left'].includes(style.textAlign)) fix = txtMesure.actualBoundingBoxLeft;
                    // // if (['end', 'right'].includes(style.textAlign)) fix = txtMesure.actualBoundingBoxRight;
                    // if (style.fillStyle || style.gradientBgc) context.fillText(txtLine, 0 - txtMesure.actualBoundingBoxLeft, lineIdx*fontSize - txtMesure.actualBoundingBoxAscent);
                    if (style.fillStyle || style.gradientBgc) context.fillText(txtLine, 0 - fix, lineIdx*fontSize);
                    if (style.strokeStyle) context.strokeText(txtLine, 0 - fix, lineIdx*fontSize);
                });
            } else {
            ...
            if (style.clear) {
                // if (opts.circleDegries) {
                //     context.arc(0, 0, renderW/2, 0, 360);
                // }
                context.clip();
                ...
                // context.rotate(0);
                // context.scale(1, 1);
                context.clearRect(0, 0, this.canvasSize.dprW, this.canvasSize.dprH);
            } else {
    ...
    render(cb) {
        ...
        if (!this.isMatrixBoard) {
            ...
            dataToRender.unshift(...this.getStaticShapesToRender());
    ...
    convertBoardSizeToCanvasSize(size = { w: 1, h: 1, x: 0, y: 0, fontSize: 0 }, isFromCanvasPos = false) {
    ...
    getDrawOpts(opts = { x: 0, y: 0, w: 1, h: 1, fontSize: 0 }) {
        ...
        return {
            ...
            ...this.convertBoardSizeToCanvasSize({ w: opts.w, h: opts.h, fontSize: opts.fontSize }),
            centerX: centerPos.x,
            centerY: centerPos.y,
    ...
    static getOffsetPosFomDomEvent(ev, isClientPos) {
        ...
        return utilsLib.getPosOnTargetElementFromMouseEvent(ev);
        const touches = ev.touches?.[0] || ev.changedTouches?.[0];
        return (typeof ev.offsetX === 'number')
            ? { x: ev.offsetX, y: ev.offsetY }
            : {
                x: touches.clientX - touches.target.offsetLeft,
                y: touches.clientY - touches.target.offsetTop
            };
    }
    ...
    calcCanvasSize() {
        ...
        return {
            ...
            h: renderSize.h,
            dpr: dpr,
            dprW: renderSize.w * dpr,
            dprH: renderSize.h * dpr,
        }
    ...
    getImg() {
        const elForImg = document.createElement('div');
        const highQualityService = new CanvasService(JSON.parse(JSON.stringify(this.board)), {...JSON.parse(JSON.stringify(this.canvasOpts)), selector: elForImg}, this.getCellContectCb);
        highQualityService.zoomLevel = this.zoomLevel;
        highQualityService.rootRenderPos = this.rootRenderPos;
        elForImg.style.position = 'fixed';
        elForImg.style.top = '-100%';
        elForImg.style.right = '-100%';
        elForImg.style.opacity = '0';
        elForImg.style.width = highQualityService.boardSize.w * 2 + 'px';
        elForImg.style.height = highQualityService.boardSize.h * 2 + 'px';
        document.body.appendChild(elForImg);
        return new Promise((resolve, reject) => {
            highQualityService.render(() => {
                setTimeout(() => {
                    const dataUrl = highQualityService.elCanvas.toDataURL('image/png');
                    document.body.removeChild(elForImg);
                    // return dataUrl;
                    resolve(dataUrl);
                }, 0);
            });
    ...
    async downloadImg() {
        ...
        linkEl.href = await this.getImg();
        linkEl.download = this.id + '.png';
    ...
    static getrenderAreaItemForTxtItem = (textCanvasItem) => {
      if (!textCanvasItem.text) return null;
      const _linesCount = textCanvasItem.text.split('\n').length;
      const areaItem = { x: textCanvasItem.x, y: textCanvasItem.y, h: textCanvasItem.fontSize*_linesCount, w: Math.max(...textCanvasItem.text.split('\n').map(_ => Utils.getTextPxSize(_, textCanvasItem.fontSize, textCanvasItem.style.fontFamily))) };
      if (textCanvasItem.style?.textAlign === 'center') areaItem.x -= areaItem.w / 2;
      if (textCanvasItem.style?.textAlign === 'end') areaItem.x -= areaItem.w;
      return areaItem;
    }
}
...
const Utils = {
    ...
    getTextPxSize: function(text, size, fontStyle = 'Arial') {
