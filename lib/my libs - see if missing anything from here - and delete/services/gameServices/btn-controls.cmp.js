...
export default function createBtnsController(cbFunc, speed = 100, parentSelector = 'body', withActions = true, styleTemplate = _getDefaultStyle()) {
    const template = {
        template: `
            <div class="mobile-controllers">
                ...
                ${withActions && `<div class="actions-btns">
                    <button value="Space" class="action-a">A</button>
                    <button value="Enter" class="action-b">B</button>
                </div>` || ''}
            </div>
        `
    }.template;
    ...
    el.querySelectorAll('button').forEach(elBtn => {
...
function _getDefaultStyle() {
    const em = elementService.makeEmHelperFunc(16);
    return {
        template: `
            <style>
                .mobile-controllers {
                ...
                .mobile-controllers .arrow-btns {
                    display: grid;
                    grid-template-rows: repeat(3, ${em(40)});
                    grid-template-columns: repeat(3, ${em(40)});
                    <!-- column-gap: 25px; -->
                }
                .mobile-controllers .arrow-btns >* {
                    background: none;
                ...
                .actions-btns .action-a {
                    margin-top: ${em(40)}
                }
                .actions-btns .action-b {
                    margin-bottom: ${em(40)}
                }
