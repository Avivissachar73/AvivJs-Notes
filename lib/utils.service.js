...
function createItemsCacher(idField = 'id') {
  const cachedItems = [];
  function useCache(item) {
    if (!item) return item;
    const exists = cachedItems.find(c => c[idField].toString() === item[idField].toString());
    if (exists) return exists;
    cachedItems.push(item);
    return item;
  }
  return { cachedItems, useCache };
}

function traverseConditionsMap(conditionsMap = [{value: '', conditions: [{txt: '', ignoreCase: false, method: ''}]}], testValue = '') {
  for (let conditionsData of conditionsMap) {
      const valueToReturn = conditionsData.value;
      const isValid = conditionsData.conditions.find(currConds => {
          return currConds.every(c => {
              let _testVal = testValue;
              let _condTxt = c.txt;
              if (c.ignoreCase || true) {
                  _testVal = _testVal.toLowerCase();
                  _condTxt = _condTxt.toLowerCase();
              }
              if (c.method === 'includes') return _testVal.includes(_condTxt);
              if (c.method === 'startsWith') return _testVal.startsWith(_condTxt);
              if (c.method === 'equals') return _testVal === _condTxt;
              return false;
          });
      }) || conditionsData.defualt;
      if (isValid) return valueToReturn;
  }
  return undefined;
}
...
function slugIt(str = '') {
    ...
    const forbidenChars = [...`!@#$%^&*()+='<>Q~{}|;[]?"`.split(''), '`', '\\'];
    const spaceChars = ',._/'.split('');
    forbidenChars.forEach(char => {
        slug = slug.split(char).join('-');
    });
    spaceChars.forEach(char => {
        slug = slug.split(char).join('-');
    });
...
function getPosOnTargetElementFromMouseEvent(ev) {
    const point = ev.point?.[0] || ev.changedTouches?.[0] || ev;
    const rect = ev.target.getBoundingClientRect();
    return { x: point.clientX - rect.left, y: point.clientY - rect.top }
    // const el = ev.target;
    // const elRect = el.getBoundingClientRect();
    // return {
    //     x: ev.pageX - elRect.left - window.scrollX,
    //     y: ev.pageY - elRect.top - window.scrollY
    // }
}
...
const _Utils = {
    ...
    trimHtmlTxt,
    createItemsCacher,
    traverseConditionsMap,
    slugIt,
    getPosOnTargetElementFromMouseEvent,
