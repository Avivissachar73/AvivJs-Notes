...
window.AvivJs = (() => {
    ...
    const routerServiceCoPN = 'routerService';
    const renderServiceCoPN = 'renderService';
    const renderCoPN = 'render';
    ...
    const ComponentService = (() => {
        ...
        const CmpUtils = {
            ...
            getElType(template) {
                if (template[0] === '<' && template[template.length-1] === '>') {
                    if (template[1] === '/') {
                        return template.split('/')[1].split('\n').join(' ').split(' ')[0].split('>')[0];
                    } 
                    else {
                        return template.split('<')[1].split('\n').join(' ').split(' ')[0].split('>')[0].split('/')[0];
                    }
                } else return;
            },
            ...
            createTemplateTree: (() => {
                ...
                function _getAttrs(templatePart) {
                    templatePart = templatePart.split('\n').join(' ');
                    var attrs = {};
            ...
            getCmpRefAtrrName: (cmp) => {
                return `ref_${cmp.name}_${cmp.id}`.toLowerCase();
            }
        };
        const directives = {
            /**
            ...
            dataBindingAmodel(component, tTree) {
                ...
                if (!tTree.events.change) tTree.events.change = [];
                tTree.events.change.push(generatedFunc);
            ...
            setBihaviorAttrs(component, tTree) {
                for (let realAttrname in behaviorAttrs) {
                    const customAttrName = behaviorAttrs[realAttrname];
                    if (!tTree.attributes?.[customAttrName]) continue;
                    const val = CmpUtils.getDataFromScriptStr(component, tTree.attributes[customAttrName], tTree.vars)
                    delete tTree.attributes[customAttrName];
                    if (val) tTree.attributes[realAttrname] = realAttrname;
                }
                return tTree;
        ...
        return class ComponentService {
            ...
            createCmp(cmp, rootProps = {}) {
                ...
                if (cmp[i18DatanCPN]) {
                    cmp[i18nCPN] = new i18nService(cmp[i18DatanCPN].locale, cmp[i18DatanCPN].messages);
                    cmp[$tSPN] = cmp[i18nCPN].$t.bind(cmp[i18nCPN]);
                } else if (cmp[RootPropsCPN][i18nCPN]) {
                    cmp[i18nCPN] = cmp[RootPropsCPN][i18nCPN];
                    cmp[$tSPN] = cmp[RootPropsCPN][i18nCPN].$t;
                }
                let context = {};
                if (cmp[StoreCPN]) {
                ...
                context[routerServiceCoPN] = this.RouterService;
                context[renderServiceCoPN] = this.RenderService;
                context[renderCoPN] = () => this.RenderService.selectiveRender(cmp)
        ...
        static getPathFromRouteItem(routeItem, routes, basePath = '') {
            for (let route of routes) {
                ...
                if (route.name === routeItem.name) {
                    ...
                    let childRes = this.getPathFromRouteItem(routeItem, route.children, currPath);
    ...
    class RenderService {
        ...
        static renderElement_noVD(cmp, evManager, selector, debug) {
            ...
            if (oldEl) {
                try { // TODO: NO TRY CATCH, FIX THIS BUG;
            ...
            setTimeout(() => { // TO MAKE IT ASYNC!; // TODO: dont use setTimeoutl
    ...
    class i18nService {
        constructor(locale = 'en', messages = { en: {  } }) {
            this.messages = messages;
            this.locale = locale;
        }
        defaultLocale = 'en';
        setLocale = localToSet => {
            this.locale = localToSet;
        }
        $t = (key, valIfMissed = '') => {
            const { locale, messages : transs } = this;
            return Utils.getDeepVal(transs[locale], key) || Utils.getDeepVal(transs[this.defaultLocale], key) || valIfMissed || key;
        }
    }
    ...
    class RouterLink {
        ...
        getters = {
            actualUrl() {
                ...
                return this.context.routerService.constructor.getPathFromRouteItem(this.url, this.context.routerService.Routes) || '404';
            },
            link() {
                return '#' + this.actualUrl;
            },
            exact() {
                return this.actualUrl === this.context.router.path;
            },
            active() {
                return this.context.router.path.substring(0, this.actualUrl.length) === this.actualUrl;
            },
            extraClass() {
                let res = '';
                res += ` ${this.className || ''}`;
