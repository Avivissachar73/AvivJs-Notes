...
const validate = (Interface, data, isOptionalMissingVals = false, strict = true, debug = false, debugPath = 'Data') => {
    function resolve(resolvedVal = false, errMsg = '') {
        if (!resolvedVal && debug) _createError(debugPath, Interface, data, errMsg);
        return resolvedVal;
    }
    ...
    if (isInstanceOfLocalClass(Interface)) return resolve(Interface.validate(data, strict, validate));
    ...
    if (Array.isArray(Interface)) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (!validate(arrType, currData, isOptionalMissingVals, strict, debug, `${debugPath}[${i}]`)) return false;
        }
        return true;
    }
    ...
    if (Interface && typeof Interface === 'object') {
        ...
        for (const key in Interface) {
            ...
            // }
            if (!validate(currType, currData, isOptionalMissingVals, strict, debug, `${debugPath}.${key}`)) return false;
        }
        return true;
    }
...
const _createError = (debugPath, type, data, msg = '') => {
